 ...
 * <p>
 * 准备:
 * 1.如果顶点v在树中marked[v]=true
 * 2.一条队列mst来保存最小生成树中的边
 * 3.MinPQ来根据权重保存所有的边
 * 4.横切边的集合:
 * 要维护一个包含所有横切边的集合,就要将连接这个顶点和其他所有不在树中的顶点的边加入优先队列(用marked[]来识别这样的边).
 * 5.连接新加入树中的顶点与其他已经在树中的顶点中的所有边都失效了,这样的边就不是横切边了.
 *
...
public class LazyPrimMST {
     ...
     *
     * 1. 扫描起始顶点s,标记s,并将s关联的所有边添加到优先队列中
     * 2. 开始迭代,迭代条件为优先队列为空,迭代步为移除优先队列中的最小值.
     * 3. 迭代步:
     * - 获得pq中的最小值
     * - 如果这个最小的边e已经失效,那么说明这个边对应的两个结点已经假如到了最小生成树中.
     * - 如果这个边还没有失效,那么将这个边加入到mst中,继续分别扫描这个边对应的没被标记过结点v或w.
     *
     * 当迭代完成时,即所有的结点均加入了mst中,并验证了所有的边.
     *
     * @param G
     * @param s
     */
     ...
     * 先标记顶点v为以访问过.
     * 对顶点v的所有边进行扫描,如果v对应的w顶点还没有标记过,将这个边插入优先队列
     *
     * @param G
     * @param v
     */
