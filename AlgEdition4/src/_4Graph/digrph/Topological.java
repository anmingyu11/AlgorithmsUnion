...
import _4Graph.sp.EdgeWeightedDigraph;
import _4Graph.sp.EdgeWeightedDirectedCycle;
 ...
 * 必须没有有向环,
 * 因为拓扑排序解决的是一个优先级调度问题,
 * 假如任务x必须在y之前完成,y必须在z之前完成,z必须在x之前完成.
 * 那么矛盾,有向环正是这种情况.
 *
 * 所以,必须先检测是否有有向环,如果有有向环,则无法进行拓扑排序.
 * 一幅有向图含有的环的数量可能是图的大小的指数级别,因此只需要找一个环,而不是所有的环.
 *
...
public class Topological {
    ...
    public Topological(EdgeWeightedDigraph G) {
        EdgeWeightedDirectedCycle ewdc = new EdgeWeightedDirectedCycle(G);
        if (!ewdc.hasCycle()) {
            DepthFirstOrder dfo = new DepthFirstOrder(G);
            order = dfo.reversePost();
        }
